...
class SingleYearProblemVisualisation(Problem):
    ...
    async def _run(self, run_job_request, websocket):
        ...
        algorithm = self.algorithm_generator._create_nsga2_algorithm(
            Constants.ALGORITHM_SINGLE_YEAR_POP_SIZE)
    ...
    async def _output_data(self, opt_data_frame, all_data_frame, websocket):
        if self.config.output_graphs_to_file:
            # Design Space        
            design_space_graph = self.graph_generator._generate_design_space_graph_single_year(
            ...
            # Objective Space
            objective_space_graph = self.graph_generator._generate_objective_space_graph_single_year(
                opt_data_frame)
            ...
            # All Individuals
            all_individuals_graph = self.graph_generator._generate_all_individuals_graph_single_year(
            ...
            # All Objectives
            all_objectives_graph = self.graph_generator._generate_all_objectives_graph_single_year(
                all_data_frame)
            ...
            # Yield Over Maturity
            yield_over_maturity_graph = self.graph_generator._generate_yield_over_maturity_graph_single_year(
                all_data_frame)
            self.results_logger._log_graph(
        ...
        await self.results_logger._log_raw_results(
            opt_data_frame, ResultsLogger.OPT_DATA_FRAME_JSON, websocket)
        await self.results_logger._log_raw_results(
            all_data_frame, ResultsLogger.ALL_DATA_FRAME_JSON, websocket)
