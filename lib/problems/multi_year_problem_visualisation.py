...
class MultiYearProblemVisualisation(Problem):
    ...
    async def _run(self, run_job_request, websocket):
        ...
        await self.results_logger._run_started(Constants.JOB_TYPE_MULTI_YEAR, run_job_request.job_id, websocket)
        ...
        self.results_logger._log_problem_entry(opt_data_frame.sort_values(Constants.YIELD_HA, ascending=False))
        ...
        await self.results_logger._run_ended(Constants.JOB_TYPE_MULTI_YEAR, self.job_id, websocket)
    ...
    def _handle_evaluate_value_for_population(self, population_value, out, results):
        ...
        outputNames = [Constants.FAILURE_RISK_YIELD_HA, Constants.OUTPUT_NAME_YIELD_HA]
    ...
    async def _output_data(self, opt_data_frame, all_data_frame, websocket):
        if self.config.output_graphs_to_file:
            ...
            self.results_logger._log_graph(design_space_graph, ResultsLogger.DESIGN_SPACE_GRAPH_JSON, ResultsLogger.DESIGN_SPACE_GRAPH_HTML)

            # Objective Space
            objective_space_graph = self.graph_generator._generate_objective_space_graph_multi_year(opt_data_frame)
            self.results_logger._log_graph(objective_space_graph, ResultsLogger.OBJECTIVE_SPACE_GRAPH_JSON, ResultsLogger.OBJECTIVE_SPACE_GRAPH_HTML)

            # All Individuals
            all_individuals_graph = self.graph_generator._generate_all_individuals_graph_multi_year(all_data_frame, self.bounds())
            self.results_logger._log_graph(all_individuals_graph, ResultsLogger.ALL_INDIVIDUALS_GRAPH_JSON, ResultsLogger.ALL_INDIVIDUALS_GRAPH_HTML)

            # All Objectives
            all_objectives_graph = self.graph_generator._generate_all_objectives_graph_multi_year(all_data_frame)
            self.results_logger._log_graph(all_objectives_graph, ResultsLogger.ALL_OBJECTIVES_SPACE_GRAPH_JSON, ResultsLogger.ALL_OBJECTIVES_SPACE_GRAPH_HTML)
        ...
        await self.results_logger._log_raw_results(Constants.JOB_TYPE_MULTI_YEAR, self.job_id, opt_data_frame, websocket)
        await self.results_logger._log_raw_results(Constants.JOB_TYPE_MULTI_YEAR, self.job_id, all_data_frame, websocket)
