...
if prb.genNumber <= 5: 
    ...
    for i in range(0, len(prb.all_df), prb.popSize):
        fig1= px.scatter(prb.all_df[i: i+ prb.popSize], x="EndJuvtoFI Thermal Time (DD)", y="Fertile Tiller Number")
    ...
    for i in range(len(groupsInterest)):
        fig1= px.scatter(prb.all_df[groupsInterest[i]: (groupsInterest[i]+ prb.popSize)], x="EndJuvtoFI Thermal Time (DD)", y="Fertile Tiller Number")
...
fig.update_layout(font_family='Courier New', font_size = 16, title_font_color = 'black', title="Variables over Generations",
                title_x=0.5, template='plotly_white', width=1400, height=700)
fig.update_xaxes(range=[prb.xl[0], prb.xu[0]], title_text = "EndJuvtoFI Thermal Time (DD)", gridcolor='lightgray', mirror= True, ticks='outside', showline=True,linecolor= 'lightgray')
fig.update_yaxes(range=[prb.xl[1], prb.xu[1]], title_text = "Fertile Tiller Number", gridcolor='lightgray', mirror= True, ticks='outside', showline=True,linecolor= 'lightgray')
fig.show()
